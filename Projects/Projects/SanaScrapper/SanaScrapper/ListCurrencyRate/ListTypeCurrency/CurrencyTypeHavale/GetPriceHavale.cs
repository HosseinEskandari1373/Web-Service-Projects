...
namespace WebScrappingHtmlPage
{
    /*
       1- Draft = حواله ارزی
    */
    public class GetPriceHavale
    {
        public static List<decimal> GetPriceDraft()
        {
            //Retrive CleanTextSiteEskenas
            string[] CleanLineDraft = CleanTextSiteHavale.CleanTextDraft();

            /*
                1- از آنجایی که قیمت ارز در سایت گاهی مقدار "--" را به خود اختصاص می دهد، بنابراین
                  . دچار مشکل خواهد شد Decimal در تبدیل رشته قیمت به
            ...
            */
            List<decimal> PriceDraft = new List<decimal>();

            // i = 13 شروع حلقه از سطر دوم جدول یعنی خانه
            for (int i = 13; i <= CleanLineDraft.Length; i++)
            {
                ...
                for (int j = 0; j < 2; j++)
                {
                    ...
                    */
                    bool Result = CleanLineDraft[i + j].Contains(",");

                    if (Result == true || CleanLineDraft[i + j].Length > 2)
                    {
                        PriceDraft.Add(Convert.ToDecimal(decimal.Parse(CleanLineDraft[i + j])));
                    }
                    else
                    {
                        PriceDraft.Add(-1);
